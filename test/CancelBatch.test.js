...
contract("WeiDex", function ([_, maker, taker]) {
    ...
    context("Cancel batch orders", function () {
        ...
        it("should cancel batch orders successfully", async function () {
            tradeTxResult = await contract.cancelMultipleOrders([firstOrder, secondOrder], [firstOrderSig, secondOrderSig], {
                from: maker
            });
        });

        it("should update orders status", async function () {
            const firstOrderInfo = await contract.getOrderInfo(firstOrderTakerReceivedAmount, firstOrder);
            expect(firstOrderInfo["status"]).to.be.eq("6"); // cancel status

            const secondOrderInfo = await contract.getOrderInfo(secondOrderTakerReceivedAmount, secondOrder);
        ...
        it("should update cancel status", async function () {
            ...
            const secondOrderPrefixedHash = await contract.getPrefixedHash(secondOrder);
            const cancelled = await contract.getCancels([firstOrderPrefixedHash, secondOrderPrefixedHash]);
        ...
        it("should fail when maker is invalid", async function () {
            await shouldFail.reverting.withMessage(
                contract.cancelMultipleOrders([firstOrder, secondOrder], [firstOrderSig, secondOrderSig], { from: taker }),
                "INVALID_SIGNER"
            );
        });
