...
contract("WeiDex", function ([_, maker, taker]) {
    ...
    context("Batch trade failure", function () {
        before(async function () {
            ...
            validOrderTakerReceivedAmount = validOrder[0].mul(validOrder[2]).div(validOrder[1]);
            invalidOrderTakerReceivedAmount = invalidOrder[0].mul(invalidOrder[2]).div(invalidOrder[1]);
        });
        ...
        it("should not update orders fill state", async function () {
            ...
            const result = await contract.getFills([validOrderHash, invalidOrderHash]);
            expect(result[0].toString()).to.be.eq("0");
            expect(result[1].toString()).to.be.eq("0");
        });

        it("should not update orders status", async function () {
            const firstOrderResult = await contract.getOrderInfo(validOrderTakerReceivedAmount, validOrder);
            expect(firstOrderResult["status"]).to.be.eq("3"); // fillable status

            const secondOrderResult = await contract.getOrderInfo(invalidOrderTakerReceivedAmount, invalidOrder);
