...
contract("WeiDex", function ([_, maker, taker]) {
    ...
    context("Batch trade", function () {
        before(async function () {
            ...
            await contract.allowOrRestrictMethod("0x6db281564fe9547306996d6f77552aebb6a1b3451dd77b0e3cd65337b6741ad2", true)
            ...
            firstOrderTakerReceivedAmount = firstOrder[0].mul(firstOrder[2]).div(firstOrder[1]);
            secondOrderTakerReceivedAmount = secondOrder[0].mul(secondOrder[2]).div(secondOrder[1]);
        });
        ...
        it("should update maker ETH balance", async function () {
            expect(makerBalanceAfter.eth.toString()).to.be.eq(
                makerBalanceBefore.eth.add(firstOrder[2]).add(secondOrder[2]).toString()
            );
        });

        it("should update maker Token balance", async function () {
            expect(makerBalanceAfter.token.toString()).to.be.eq(
                makerBalanceBefore.token.sub(firstOrderTakerReceivedAmount).sub(secondOrderTakerReceivedAmount).toString()
            );
        });

        it("should update taker ETH balance", async function () {
            expect(takerBalanceAfter.eth.toString()).to.be.eq(
                takerBalanceBefore.eth.sub(firstOrder[2]).sub(secondOrder[2]).toString()
            );
        });

        it("should update taker Token balance", async function () {
            expect(takerBalanceAfter.token.toString()).to.be.eq(
                takerBalanceBefore.token.add(firstOrderTakerReceivedAmount).add(secondOrderTakerReceivedAmount).toString()
            );
        });
        ...
        it("should update orders status", async function () {
            const firstOrderResult = await contract.getOrderInfo(firstOrderTakerReceivedAmount, firstOrder);
            expect(firstOrderResult["status"]).to.be.eq("5"); // fully filled status

            const secondOrderResult = await contract.getOrderInfo(secondOrderTakerReceivedAmount, secondOrder);
